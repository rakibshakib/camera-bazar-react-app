            ...
            fetch(url, {
                ...
                .then(data => {
                    if (data.deletedCount > 0) {
                        ...
                        const remainingProducts = allOrders.filter(item => item._id !== id);
    ...
    return (
        <div className='container mx-auto my-5'>
            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-1 my-5 py-5 mx-8 justify-items-center md:border-2 rounded-3xl text-base md:text-xl font-semibold'>
                <h2 className='flex items-center'> <GrUserAdmin className='inline-block mr-2' /> UserType: <span className=' md:ml-3'> Admin </span></h2>
            ...
            <div className='flex flex-wrap mx-2'>
               {   
                    ...
                    : 
                   allOrders.map(item=> <AllOrderCard
                   key={item._id}
                   item={item}
